...
const createindex = async () => {
  let content = { index: '', header: '', articles: '', footer: '' };
  ...
  file.on('end', () => {
    file.destroy;
    content.index = content.index.replace('{{header}}', content.header);
    content.index = content.index.replace('{{articles}}', content.articles);
    content.index = content.index.replace('{{footer}}', content.footer);
    const indexdist = path.join(distdir, 'index.html');
    const indexfile = new fs.WriteStream(indexdist, 'utf8');
    indexfile.write(content.index);
  });
};
